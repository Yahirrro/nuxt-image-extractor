...
module.exports = function Extract(moduleOptions) {
  ...
  async function process(page) {
    ...
    for (const match of matches) {
      ...
      let matchURL = match[0]
      if(match[0].match(/\)$/)) matchURL = matchURL.slice( 0, -1 )
...
async function saveRemoteImage(url, path) {
  ...
  const resizeRemoteImage = await sharp().jpeg({ quality: 50, progressive: true }).resize(1280)
  return await new Promise((resolve, reject) => {
    res.body.pipe(resizeRemoteImage).pipe(fileStream)
    res.body.on('error', (err) => {
      reject(err)
    })
    resizeRemoteImage.on('error', (err) => {
      reject(err)
    })
